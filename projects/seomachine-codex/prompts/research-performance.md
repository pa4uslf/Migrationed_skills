...
> 迁移来源：`seomachine-codex/.claude/commands/research-performance.md`

> 用法：在 Codex 中把这份文件当作工作流模板使用，而不是当作 slash command。

> 额外建议：优先结合本目录下的 `skills/`、`scripts/` 和 `context/`，不要只照搬文案流程。
